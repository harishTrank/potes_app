...
import { createContactApi, editContactApi } from "../../../store/Services/Others";
...
const CollapsibleSection: any = ({ title, children, initiallyOpen = false }: any) => {
  ...
  return (
    <View style={styles.collapsibleSection}>
      <TouchableOpacity onPress={toggleOpen} style={styles.sectionHeader}>
        ...
        <Feather name={isOpen ? "chevron-up" : "chevron-down"} size={20} color={theme.colors.greyText} />
      </TouchableOpacity>
...
const CreateContactScreen: any = ({ navigation, route }: any) => {
  ...
  const [initialContactValues, setInitialContactValues]: any = useState({
    ...
    spouseDetails: "",
    children: [{ id: `child-${Date.now()}`, name: "", birthday: undefined, details: "" }],
  ...
  useEffect(() => {
    ...
    if (type === "edit" && contactToEdit) {
      setInitialContactValues({
        ...
        children: contactToEdit.children?.map((c: any) => ({ id: c.id?.toString() || `child-${Date.now()}`, name: c.name || "", birthday: parseApiDate(c.birthday), details: c.details || "" })) || [],
        employmentHistory: contactToEdit.previous_employers?.map((e: any) => ({ id: e.id?.toString() || `emp-${Date.now()}`, name: e.name || "", details: e.details || "" })) || [],
        educationHistory: contactToEdit.universities?.map((e: any) => ({ id: e.id?.toString() || `edu-${Date.now()}`, name: e.name || "", details: e.details || "" })) || [],
        interests: contactToEdit.interests?.map((i: any) => ({ id: i.id?.toString() || `interest-${Date.now()}`, name: i.name || "" })) || [],
        customFields: contactToEdit.custom_fields?.map((cf: any) => ({ id: cf.id?.toString() || `custom-${Date.now()}`, title: cf.title || "", values: Array.isArray(cf.values) ? cf.values.map(String) : [] })) || [],
      });
  ...
  const handleFormSubmit = async (values: any, { setSubmitting, resetForm }: any) => {
    ...
    formData.append("children", JSON.stringify(values.children.map((c: any) => ({ ...c, id: undefined, birthday: formatDateToYYYYMMDD(c.birthday) }))));
    formData.append("previous_employers", JSON.stringify(values.employmentHistory.map((e: any) => ({ name: e.name, details: e.details }))));
    formData.append("universities", JSON.stringify(values.educationHistory.map((e: any) => ({ name: e.name, details: e.details }))));
    formData.append("interests", JSON.stringify(values.interests.map((i: any) => ({ name: i.name }))));
  ...
  const renderTextInput = (formikBag: any, name: string, label: string, placeholder: string, required = false, keyboardType: any = "default", multiline = false, numberOfLines = 1) => (
  ...
  const renderDateInput = (formikValues: any, _setFieldValue: Function, fieldPath: string, label: string, placeholder: string, arrayIndex?: number) => {
    ...
    return (
      <View style={styles.inputGroup}>
        ...
        <TouchableOpacity style={styles.dateInput} onPress={() => openDatePicker(fieldPath, displayValue, arrayIndex)}>
  ...
  return (
    <DefaultBackground>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={Platform.OS === "ios" ? 0 : -150}>
        <View style={[styles.headerRow, { paddingTop: insets.top + 6 }]}>
          ...
          <Text style={styles.headerTitle}>{isEdit ? "Edit Contact" : "Create Contact"}</Text>
          <View style={styles.headerRight} />
        </View>

        <ScrollView style={styles.scroll} contentContainerStyle={{ paddingBottom: insets.bottom + 30 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          <Formik initialValues={initialContactValues} onSubmit={handleFormSubmit} enableReinitialize>
              ...
              <View style={{ paddingHorizontal: 16 }}>
                ...
                <TouchableOpacity style={styles.avatarSection} onPress={handlePickImage}>
                  <View style={styles.avatarContainer}>
                    {selectedAvatarFileUri ? (
                      ...
                      <Feather name="user-plus" size={36} color={theme.colors.greyText} />
                    )}
                ...
                <View style={styles.dateRow}>
                  <View style={[styles.card, { flex: 1, marginRight: 6 }]}>
                    ...
                    <TouchableOpacity style={styles.dateInputInline} onPress={() => openDatePicker("birthday", values.birthday)}>
                  ...
                  <View style={[styles.card, { flex: 1, marginLeft: 6 }]}>
                    ...
                    <TouchableOpacity style={styles.dateInputInline} onPress={() => openDatePicker("anniversary", values.anniversary)}>
                      <Text style={values.anniversary ? styles.dateText : styles.datePlaceholder}>
                        {values.anniversary ? dayjs(values.anniversary).format("MM/DD") : "mm/dd"}
                      </Text>
                ...
                <View style={styles.sectionsCard}>
                  <CollapsibleSection title="Family Details">
                    {renderTextInput({ values, handleChange, handleBlur }, "spouseName", "Spouse Name", "Enter spouse name")}
                    {renderDateInput(values, setFieldValue, "spouseBirthday", "Spouse Birthday", "mm/dd")}
                    {renderTextInput({ values, handleChange, handleBlur }, "spouseDetails", "Spouse Details", "Enter details", false, "default", true, 2)}
                          ...
                          {values.children.map((child: any, index: any) => (
                            <View key={child.id} style={styles.arrayCard}>
                              <View style={styles.arrayCardHeader}>
                                <Text style={styles.arrayCardTitle}>Family Member {index + 1}</Text>
                                <TouchableOpacity onPress={() => arrayHelpers.remove(index)}>
                                  <Feather name="trash-2" size={18} color={theme.colors.red} />
                                </TouchableOpacity>
                              </View>
                              {renderTextInput({ values: child, handleChange: (f: string) => (v: string) => setFieldValue(`children[${index}].${f}`, v), handleBlur }, "name", "Name", "Enter name")}
                              {renderDateInput(values, setFieldValue, `children[index].birthday`, "Birthday", "mm/dd", index)}
                            </View>
                          ))}
                          <TouchableOpacity style={styles.addBtn} onPress={() => arrayHelpers.push({ id: `child-${Date.now()}`, name: "", birthday: undefined, details: "" })}>
                  ...
                  <CollapsibleSection title="Employment">
                    <FieldArray name="employmentHistory">
                      {(arrayHelpers: FieldArrayRenderProps) => (
                        <View>
                          {values.employmentHistory.map((job: any, index: any) => (
                            <View key={job.id} style={styles.arrayCard}>
                              <View style={styles.arrayCardHeader}>
                                <Text style={styles.arrayCardTitle}>Employment {index + 1}</Text>
                                <TouchableOpacity onPress={() => arrayHelpers.remove(index)}>
                                  <Feather name="trash-2" size={18} color={theme.colors.red} />
                                </TouchableOpacity>
                              </View>
                              {renderTextInput({ values: job, handleChange: (f: string) => (v: string) => setFieldValue(`employmentHistory[${index}].${f}`, v), handleBlur }, "name", "Employer", "Employer name")}
                              {renderTextInput({ values: job, handleChange: (f: string) => (v: string) => setFieldValue(`employmentHistory[${index}].${f}`, v), handleBlur }, "details", "Details", "Role/position", false, "default", true, 2)}
                            </View>
                          ))}
                          <TouchableOpacity style={styles.addBtn} onPress={() => arrayHelpers.push({ id: `emp-${Date.now()}`, name: "", details: "" })}>
                            <Text style={styles.addBtnText}>+ Add Employment</Text>
                          </TouchableOpacity>
                        </View>
                      )}
                    </FieldArray>
                  ...
                  <CollapsibleSection title="Education">
                    <FieldArray name="educationHistory">
                      {(arrayHelpers: FieldArrayRenderProps) => (
                        <View>
                          {values.educationHistory.map((edu: any, index: any) => (
                            <View key={edu.id} style={styles.arrayCard}>
                              <View style={styles.arrayCardHeader}>
                                <Text style={styles.arrayCardTitle}>Education {index + 1}</Text>
                                <TouchableOpacity onPress={() => arrayHelpers.remove(index)}>
                                  <Feather name="trash-2" size={18} color={theme.colors.red} />
                                </TouchableOpacity>
                              </View>
                              {renderTextInput({ values: edu, handleChange: (f: string) => (v: string) => setFieldValue(`educationHistory[${index}].${f}`, v), handleBlur }, "name", "Institution", "University name")}
                              {renderTextInput({ values: edu, handleChange: (f: string) => (v: string) => setFieldValue(`educationHistory[${index}].${f}`, v), handleBlur }, "details", "Details", "Degree/program", false, "default", true, 2)}
                            </View>
                          ))}
                          <TouchableOpacity style={styles.addBtn} onPress={() => arrayHelpers.push({ id: `edu-${Date.now()}`, name: "", details: "" })}>
                    ...
                    <FieldArray name="interests">
                      {(arrayHelpers: FieldArrayRenderProps) => (
                        <View>
                          {values.interests.map((interest: any, index: any) => (
                            <View key={interest.id} style={styles.interestRow}>
                              <TextInput
                                ...
                                placeholderTextColor={theme.colors.grey}
                                value={interest.name}
                                onChangeText={handleChange(`interests[${index}].name`)}
                              />
                              <TouchableOpacity onPress={() => arrayHelpers.remove(index)}>
                                <Feather name="trash-2" size={18} color={theme.colors.red} />
                              </TouchableOpacity>
                          ...
                          <TouchableOpacity style={styles.addBtn} onPress={() => arrayHelpers.push({ id: `interest-${Date.now()}`, name: "" })}>
                  ...
                  <CollapsibleSection title="More Details">
                    <Text style={styles.moreDetailsHint}>
                      A catch-all for anything else — preferred contact method, how you met, notes on their personality, etc.
                    </Text>
                        ...
                        <View>
                          {values.customFields.map((cf: any, cfIndex: any) => (
                            <View key={cf.id} style={styles.arrayCard}>
                              <View style={styles.arrayCardHeader}>
                                <TextInput
                                  style={[styles.input, { flex: 1, marginRight: 8, marginBottom: 0, height: 40 }]}
                                  placeholder="Field title"
                                  value={cf.title}
                                  onChangeText={handleChange(`customFields[${cfIndex}].title`)}
                                  placeholderTextColor={theme.colors.grey}
                                />
                                <TouchableOpacity onPress={() => customFieldArrayHelpers.remove(cfIndex)}>
                                  <Feather name="x" size={20} color={theme.colors.red} />
                                </TouchableOpacity>
                              </View>
                              <FieldArray name={`customFields[${cfIndex}].values`}>
                                {(valueHelpers: any) => (
                                  <View>
                                    {cf.values.map((val: any, valIdx: any) => (
                                      <View key={valIdx} style={styles.interestRow}>
                                        <TextInput
                                          style={[styles.input, { flex: 1, marginRight: 8, marginBottom: 0 }]}
                                          placeholder="Value"
                                          value={val}
                                          onChangeText={handleChange(`customFields[${cfIndex}].values[${valIdx}]`)}
                                          placeholderTextColor={theme.colors.grey}
                                        />
                                        <TouchableOpacity onPress={() => valueHelpers.remove(valIdx)}>
                                          <Feather name="minus-circle" size={18} color={theme.colors.red} />
                                        </TouchableOpacity>
                                      </View>
                                    ))}
                                    <TouchableOpacity style={styles.addBtn} onPress={() => valueHelpers.push("")}>
                          ...
                          <TouchableOpacity style={styles.addBtn} onPress={() => customFieldArrayHelpers.push({ id: `custom-${Date.now()}`, title: "", values: [""] })}>
                ...
                {/* Save Button */}
                <TouchableOpacity style={[styles.saveBtn, isSubmitting && styles.btnDisabled]} onPress={() => handleSubmit()} disabled={isSubmitting}>
                  {isSubmitting ? (
                    ...
                    initialDate={
                      activeDateField
                        ? activeDateField.index !== undefined
                          ? values[activeDateField.path.split("[index]")[0]]?.[activeDateField.index]?.[activeDateField.path.split(".")[1]]
                          : values[activeDateField.path]
                        : undefined
                    }
                    onClose={() => { setMonthDayVisible(false); setActiveDateField(null); }}
                    onConfirm={(date: Date) => {
                      if (!activeDateField) return;
                      if (activeDateField.index !== undefined) {
                        const arrayName = activeDateField.path.split("[index]")[0];
                        const fieldName = activeDateField.path.split(".")[1];
                        setFieldValue(`${arrayName}[${activeDateField.index}].${fieldName}`, date);
                      } else {
...
const styles = StyleSheet.create({
  ...
  addPhotoText: { fontSize: 11, fontFamily: "Poppins-SemiBold", color: theme.colors.greyText, marginTop: 8, letterSpacing: 0.5 },
  card: {
  ...
  inputLabel: { fontSize: 12, fontFamily: "Poppins-Medium", color: theme.colors.greyText, marginBottom: 6 },
  input: {
    backgroundColor: theme.colors.lightCard,
    borderRadius: 8,
  ...
  collapsibleSection: { borderTopWidth: 1, borderTopColor: theme.colors.border },
  sectionHeader: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  sectionHeaderText: { fontSize: 15, fontFamily: "Poppins-SemiBold", color: theme.colors.text },
  sectionContent: { paddingHorizontal: 14, paddingBottom: 14 },
  arrayCard: { backgroundColor: theme.colors.lightCard, borderRadius: 8, padding: 10, marginBottom: 10 },
  arrayCardHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 10 },
  arrayCardTitle: { fontSize: 13, fontFamily: "Poppins-SemiBold", color: theme.colors.text },
  addBtn: { paddingVertical: 8 },
  addBtnText: { fontSize: 14, fontFamily: "Poppins-Medium", color: theme.colors.primary },
  interestRow: { flexDirection: "row", alignItems: "center", marginBottom: 8 },
  moreDetailsHint: { fontSize: 12, fontFamily: "Poppins-Regular", color: theme.colors.greyText, marginBottom: 12, fontStyle: "italic" },
  saveBtn: {
    backgroundColor: theme.colors.primary,
    borderRadius: 12,
    paddingVertical: 16,
